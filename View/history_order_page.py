...
class History_page(Navigation):
    ...
    def view(self, page: Page, params: Params, basket: Basket):
        ...
        Category_title = Text("История заказов", size=70, weight=FontWeight.BOLD, color=update_colors()["text_color"])
        ...
        for i in data_list:
            if i[2] == "pending" or i[2] == "On the way":
                continue
            if i[2] == "canceled":
                cart = OrderCard(page, i[0], i[1], i[2], i[3], i[4], self.user_id, "c")
                card_list.append(cart.build())
            else:
                cart = OrderCard(page, i[0], i[1], i[2], i[3], i[4], self.user_id, "d")
                card_list.append(cart.build())
